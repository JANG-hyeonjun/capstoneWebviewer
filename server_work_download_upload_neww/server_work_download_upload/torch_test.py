...
input_slide = np.array(Image.open(f"/mnt/pathology/WSI_image/{folder_name}/original/{inputs}"))
...
for filename in os.listdir(os.path.join('/mnt/pathology/pth_20/')):
    if inputs.split('.')[0].split('_')[-1] in filename:
        resnet_step1.load_state_dict(torch.load(f'/mnt/pathology/pth_20/{filename}'))
        break
...
fig.savefig('/home/hufsbme/Desktop/capstoneWebviewer/server_work_download_upload_neww/server_work_download_upload/public/img/result.png')
